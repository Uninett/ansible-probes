...
wget -rnpH -P/root/scripts --cut-dirs=1 --quiet --reject "index.html*" $SCRIPTSURL
...
nohup sudo ${SCRIPT_DIR}measure_loop.sh >/dev/null 2>&1 &

# probe will now remain in a loop cycle
